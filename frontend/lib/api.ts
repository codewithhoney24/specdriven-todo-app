'use client';

import axios from 'axios';
import { Task } from '@/types/task';

// Define the base API service with proper endpoints
const apiService = {
  tasks: {
    // Get all tasks for the authenticated user
    getAll: async (userId?: string) => {
      const token = localStorage.getItem('auth-token');

      // Get user ID from token if not provided
      let effectiveUserId = userId;
      if (!effectiveUserId) {
        // Import the function to get user ID from token
        const tokenUserId = localStorage.getItem('auth-token') ?
          (() => {
            try {
              const tokenParts = localStorage.getItem('auth-token')!.split('.');
              if (tokenParts.length !== 3) return null;
              const base64 = tokenParts[1].replace(/-/g, '+').replace(/_/g, '/');
              const paddedBase64 = base64.padEnd(base64.length + (4 - base64.length % 4) % 4, '=');
              const payload = JSON.parse(atob(paddedBase64));
              return payload.sub || null;
            } catch (e) {
              console.error('Error extracting user ID from token:', e);
              return null;
            }
          })() : null;

        if (!tokenUserId) {
          throw new Error('User ID is required to get tasks');
        }
        effectiveUserId = tokenUserId;
      }

      // Ensure userId is properly formatted (trim any whitespace)
      if (!effectiveUserId) {
        throw new Error('User ID is required to get tasks');
      }
      const cleanUserId = effectiveUserId.trim();

      console.log('Making API call to get tasks for user:', cleanUserId); // Debug log
      console.log('Using token:', token ? 'Present' : 'Missing'); // Debug log

      try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/${cleanUserId}/tasks`, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
        return response;
      } catch (error: any) {
        // If we get a 403 error, it might be due to an invalid/expired token
        if (error.response?.status === 403) {
          console.log('Received 403 error, clearing token and suggesting re-authentication');
          // Clear the token to force re-login
          localStorage.removeItem('auth-token');

          // Also clear any related auth state
          if (typeof window !== 'undefined') {
            // Trigger a global event to notify other parts of the app
            window.dispatchEvent(new Event('auth-error'));
          }
        }
        throw error; // Re-throw the error to be handled by the caller
      }
    },
    getById: async (id: number, userId?: string) => {
      const token = localStorage.getItem('auth-token');

      // Get user ID from token if not provided
      let effectiveUserId = userId;
      if (!effectiveUserId) {
        const tokenUserId = localStorage.getItem('auth-token') ?
          (() => {
            try {
              const tokenParts = localStorage.getItem('auth-token')!.split('.');
              if (tokenParts.length !== 3) return null;
              const base64 = tokenParts[1].replace(/-/g, '+').replace(/_/g, '/');
              const paddedBase64 = base64.padEnd(base64.length + (4 - base64.length % 4) % 4, '=');
              const payload = JSON.parse(atob(paddedBase64));
              return payload.sub || null;
            } catch (e) {
              console.error('Error extracting user ID from token:', e);
              return null;
            }
          })() : null;

        if (!tokenUserId) {
          throw new Error('User ID is required to get a task');
        }
        effectiveUserId = tokenUserId;
      }

      // Ensure userId is properly formatted (trim any whitespace)
      if (!effectiveUserId) {
        throw new Error('User ID is required to get a task');
      }
      const cleanUserId = effectiveUserId.trim();

      try {
        return await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/${cleanUserId}/tasks/${id}`, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
      } catch (error: any) {
        // If we get a 403 error, it might be due to an invalid/expired token
        if (error.response?.status === 403) {
          console.log('Received 403 error in getById, clearing token');
          localStorage.removeItem('auth-token');

          // Also clear any related auth state
          if (typeof window !== 'undefined') {
            // Trigger a global event to notify other parts of the app
            window.dispatchEvent(new Event('auth-error'));
          }
        }
        throw error;
      }
    },
    create: async (taskData: Partial<Task>, userId?: string) => {
      const token = localStorage.getItem('auth-token');

      // Get user ID from token if not provided
      let effectiveUserId = userId;
      if (!effectiveUserId) {
        const tokenUserId = localStorage.getItem('auth-token') ?
          (() => {
            try {
              const tokenParts = localStorage.getItem('auth-token')!.split('.');
              if (tokenParts.length !== 3) return null;
              const base64 = tokenParts[1].replace(/-/g, '+').replace(/_/g, '/');
              const paddedBase64 = base64.padEnd(base64.length + (4 - base64.length % 4) % 4, '=');
              const payload = JSON.parse(atob(paddedBase64));
              return payload.sub || null;
            } catch (e) {
              console.error('Error extracting user ID from token:', e);
              return null;
            }
          })() : null;

        if (!tokenUserId) {
          throw new Error('User ID is required to create a task');
        }
        effectiveUserId = tokenUserId;
      }

      // Ensure userId is properly formatted (trim any whitespace)
      if (!effectiveUserId) {
        throw new Error('User ID is required to create a task');
      }
      const cleanUserId = effectiveUserId.trim();

      try {
        return await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/${cleanUserId}/tasks`, taskData, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
      } catch (error: any) {
        // If we get a 403 error, it might be due to an invalid/expired token
        if (error.response?.status === 403) {
          console.log('Received 403 error in create, clearing token');
          localStorage.removeItem('auth-token');

          // Also clear any related auth state
          if (typeof window !== 'undefined') {
            // Trigger a global event to notify other parts of the app
            window.dispatchEvent(new Event('auth-error'));
          }
        }
        throw error;
      }
    },
    update: async (id: number, taskData: Partial<Task>, userId?: string) => {
      const token = localStorage.getItem('auth-token');

      // Get user ID from token if not provided
      let effectiveUserId = userId;
      if (!effectiveUserId) {
        const tokenUserId = localStorage.getItem('auth-token') ?
          (() => {
            try {
              const tokenParts = localStorage.getItem('auth-token')!.split('.');
              if (tokenParts.length !== 3) return null;
              const base64 = tokenParts[1].replace(/-/g, '+').replace(/_/g, '/');
              const paddedBase64 = base64.padEnd(base64.length + (4 - base64.length % 4) % 4, '=');
              const payload = JSON.parse(atob(paddedBase64));
              return payload.sub || null;
            } catch (e) {
              console.error('Error extracting user ID from token:', e);
              return null;
            }
          })() : null;

        if (!tokenUserId) {
          throw new Error('User ID is required to update a task');
        }
        effectiveUserId = tokenUserId;
      }

      // Ensure userId is properly formatted (trim any whitespace)
      if (!effectiveUserId) {
        throw new Error('User ID is required to update a task');
      }
      const cleanUserId = effectiveUserId.trim();

      try {
        return await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/api/${cleanUserId}/tasks/${id}`, taskData, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
      } catch (error: any) {
        // If we get a 403 error, it might be due to an invalid/expired token
        if (error.response?.status === 403) {
          console.log('Received 403 error in update, clearing token');
          localStorage.removeItem('auth-token');

          // Also clear any related auth state
          if (typeof window !== 'undefined') {
            // Trigger a global event to notify other parts of the app
            window.dispatchEvent(new Event('auth-error'));
          }
        }
        throw error;
      }
    },
    delete: async (id: number, userId?: string) => {
      const token = localStorage.getItem('auth-token');

      // Get user ID from token if not provided
      let effectiveUserId = userId;
      if (!effectiveUserId) {
        const tokenUserId = localStorage.getItem('auth-token') ?
          (() => {
            try {
              const tokenParts = localStorage.getItem('auth-token')!.split('.');
              if (tokenParts.length !== 3) return null;
              const base64 = tokenParts[1].replace(/-/g, '+').replace(/_/g, '/');
              const paddedBase64 = base64.padEnd(base64.length + (4 - base64.length % 4) % 4, '=');
              const payload = JSON.parse(atob(paddedBase64));
              return payload.sub || null;
            } catch (e) {
              console.error('Error extracting user ID from token:', e);
              return null;
            }
          })() : null;

        if (!tokenUserId) {
          throw new Error('User ID is required to delete a task');
        }
        effectiveUserId = tokenUserId;
      }

      // Ensure userId is properly formatted (trim any whitespace)
      if (!effectiveUserId) {
        throw new Error('User ID is required to delete a task');
      }
      const cleanUserId = effectiveUserId.trim();
      const url = `${process.env.NEXT_PUBLIC_API_URL}/api/${cleanUserId}/tasks/${id}`;
      console.log(`Deleting task with URL: ${url}`);

      try {
        return await axios.delete(url, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
      } catch (error: any) {
        // If we get a 403 error, it might be due to an invalid/expired token
        if (error.response?.status === 403) {
          console.log('Received 403 error in delete, clearing token');
          localStorage.removeItem('auth-token');

          // Also clear any related auth state
          if (typeof window !== 'undefined') {
            // Trigger a global event to notify other parts of the app
            window.dispatchEvent(new Event('auth-error'));
          }
        }
        throw error;
      }
    },
    toggleComplete: async (id: number, completed: boolean, userId?: string) => {
      const token = localStorage.getItem('auth-token');

      // Get user ID from token if not provided
      let effectiveUserId = userId;
      if (!effectiveUserId) {
        const tokenUserId = localStorage.getItem('auth-token') ?
          (() => {
            try {
              const tokenParts = localStorage.getItem('auth-token')!.split('.');
              if (tokenParts.length !== 3) return null;
              const base64 = tokenParts[1].replace(/-/g, '+').replace(/_/g, '/');
              const paddedBase64 = base64.padEnd(base64.length + (4 - base64.length % 4) % 4, '=');
              const payload = JSON.parse(atob(paddedBase64));
              return payload.sub || null;
            } catch (e) {
              console.error('Error extracting user ID from token:', e);
              return null;
            }
          })() : null;

        if (!tokenUserId) {
          throw new Error('User ID is required to toggle task completion');
        }
        effectiveUserId = tokenUserId;
      }

      // Ensure userId is properly formatted (trim any whitespace)
      if (!effectiveUserId) {
        throw new Error('User ID is required to toggle task completion');
      }
      const cleanUserId = effectiveUserId.trim();

      try {
        return await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/api/${cleanUserId}/tasks/${id}/complete`,
          { completed },
          {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          }
        );
      } catch (error: any) {
        // If we get a 403 error, it might be due to an invalid/expired token
        if (error.response?.status === 403) {
          console.log('Received 403 error in toggleComplete, clearing token');
          localStorage.removeItem('auth-token');

          // Also clear any related auth state
          if (typeof window !== 'undefined') {
            // Trigger a global event to notify other parts of the app
            window.dispatchEvent(new Event('auth-error'));
          }
        }
        throw error;
      }
    },
    // Get subtasks for a specific task
    getSubtasks: async (taskId: number, userId?: string) => {
      const token = localStorage.getItem('auth-token');

      // Get user ID from token if not provided
      let effectiveUserId = userId;
      if (!effectiveUserId) {
        const tokenUserId = localStorage.getItem('auth-token') ?
          (() => {
            try {
              const tokenParts = localStorage.getItem('auth-token')!.split('.');
              if (tokenParts.length !== 3) return null;
              const base64 = tokenParts[1].replace(/-/g, '+').replace(/_/g, '/');
              const paddedBase64 = base64.padEnd(base64.length + (4 - base64.length % 4) % 4, '=');
              const payload = JSON.parse(atob(paddedBase64));
              return payload.sub || null;
            } catch (e) {
              console.error('Error extracting user ID from token:', e);
              return null;
            }
          })() : null;

        if (!tokenUserId) {
          throw new Error('User ID is required to get subtasks');
        }
        effectiveUserId = tokenUserId;
      }

      // Ensure userId is properly formatted (trim any whitespace)
      if (!effectiveUserId) {
        throw new Error('User ID is required to get subtasks');
      }
      const cleanUserId = effectiveUserId.trim();

      try {
        return await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/${cleanUserId}/tasks/${taskId}/subtasks`, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
      } catch (error: any) {
        // If we get a 403 error, it might be due to an invalid/expired token
        if (error.response?.status === 403) {
          console.log('Received 403 error in getSubtasks, clearing token');
          localStorage.removeItem('auth-token');

          // Also clear any related auth state
          if (typeof window !== 'undefined') {
            // Trigger a global event to notify other parts of the app
            window.dispatchEvent(new Event('auth-error'));
          }
        }
        throw error;
      }
    },
  },
};

export { apiService };
